...
head_start:
    ...
    ; sti 
    ; when sti before iret won't trigger hardware interrupt because, The hardware interrupt fails to trigger because the pushf instruction captures a "snapshot" of the processor flags while interrupts are still disabled, saving that state onto the stack. Even if sti is executed immediately afterward to enable interrupts, the subsequent iret instruction pops that original "disabled" snapshot back into the EFLAGS register, effectively overwriting the enabled state. Consequently, the task begins execution with the interrupt flag cleared, preventing the timer or any other hardware signal from breaking through.
